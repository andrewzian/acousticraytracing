...
@njit
def paraboloid_intersect(ray_pos, ray_dir, coeffs):
    """
    Check if the ray intersects with the paraboloid defined by the equation
    z = a * (x^2 + y^2) + b
    """
    a, b, d = coeffs
    o = ray_pos
...
class Paraboloid(Surface):
    def __init__(self, paraboloid_coeffs, reflection_eff=1.0):
        super().__init__(reflection_eff)
        self.paraboloid_coeffs = paraboloid_coeffs

    def intersect(self, ray):
        """
        Check if the ray intersects with the paraboloid
        """
    ...
    def normal_at(self, point):
        """
        Calculate the normal vector at a given point on the paraboloid
        """
        a, b, d = self.paraboloid_coeffs
        normal = np.array([2 * point[0], 2 * point[1], -4 * a * point[2]])
        normal /= np.linalg.norm(normal)
        return normal
