...
class RayTracingSolver:
    ...
    def plot(self, show_solution=False, num_rays_show=10, ray_colorscale='Viridis', opacity=False, saveto=None):
        ...
        if show_solution:
            ...
            if opacity:
                # Plot rays with opacity scaling by log-power
                for i in indices:
                    ray = self.solution.rays[i]
                    if len(ray.path) < 2:
                        continue
                    ...
                    # Normalize log_powers to [0, 1] for opacity
                    min_op, max_op = 0, 1.0
                    log_min, log_max = np.min(log_powers), np.max(log_powers)
                    power_min, power_max = np.min(powers), np.max(powers)
                    # opacities = min_op + (log_powers - log_min) / (log_max - log_min + 1e-12) * (max_op - min_op)
                    opacities = min_op + (powers - power_min) / (power_max - power_min + 1e-12) * (max_op - min_op)
                    # Plot each segment as a separate trace with its opacity
                    for j in range(len(path) - 1):
                        fig.add_trace(go.Scatter3d(
                            x=[path[j, 0], path[j+1, 0]],
                            y=[path[j, 1], path[j+1, 1]],
                            z=[path[j, 2], path[j+1, 2]],
                            mode='lines',
                            line=dict(
                                color='blue',  # Fixed color for all rays
                                width=3,
                            ),
                            opacity=opacities[j],
                            name=f'Ray {i}' if j == 0 else None,
                            showlegend=(j == 0)
                        ))
            else:
                # Plot rays
