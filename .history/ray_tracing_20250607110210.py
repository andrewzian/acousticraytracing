...
class RayTracingSolver:
    ...
    def plot(self, show_solution=False, num_rays_show=10):
        ...
        if show_solution:
            # Plot rays
            for ray in self.solution.rays[::max(1, int(len(self.solution.rays)/num_rays_show))]:
                if len(ray.path) < 2:
                    continue
                path = np.array(ray.path)
                # If you store time at each path point, use that; otherwise, interpolate
                if hasattr(ray, "path_times"):
                    times = np.array(ray.path_times)
                else:
                    # If not, linearly interpolate from 0 to ray.time
                    times = np.linspace(0, ray.time, len(path))
                fig.add_trace(go.Scatter3d(
                    ...
                    line=dict(
                        ...
                        colorscale='Viridis',
                        width=3,
                        colorbar=dict(title='Ray Time') if ray is self.solution.rays[0] else None  # Show colorbar once
                    ),
                    opacity=0.7,
                    name='Ray Path'
                ))
