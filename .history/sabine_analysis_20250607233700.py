"""
This script generates a dataset to analyze the accuracy of the Sabine formula 
for predicting RT60 in rooms of various dimensions and reflection coefficients.
It creates random rooms, simulates sound propagation using ray tracing, and 
compares the simulated RT60 values with those predicted by the Sabine formula.

Output is saved to 'sabine_analysis.csv'.
"""
...
for _ in tqdm(range(n)):
    x_dim = np.random.uniform(2, max_dim)
    y_dim = np.random.uniform(2, max_dim)
    z_dim = np.random.uniform(2, max_dim)
    ...
    receiver_pos = [[np.random.uniform(x_bounds[0] + 0.1, x_bounds[1] - 0.1),
                    ...
                    np.random.uniform(z_bounds[0] + 0.1, z_bounds[1] - 0.1)] for _ in range(4)]
    ...
    receivers = [Receiver(position=receiver_pos[0], radii=[0.1, 0.1, 0.1], type="virtual"), 
                 Receiver(position=receiver_pos[1], radii=[0.1, 0.1, 0.1], type="virtual"), 
                 Receiver(position=receiver_pos[2], radii=[0.1, 0.1, 0.1], type="virtual"), 
                 Receiver(position=receiver_pos[3], radii=[0.1, 0.1, 0.1], type="virtual")]
    solver = RayTracingSolver(room, sources, receivers, max_time=sabine_rt60 * 5, energy_threshold=power / num_rays * 1e-7)

    solution = solver.solve()

    rt60_threshold = solution.get_RT60(method="threshold", aggregate=True)[0]
    rt60_schroeder = solution.get_RT60(method="schroeder", aggregate=True)[0]
    sigma = 0.01 * sabine_rt60 # error from kernel will be ~1%
