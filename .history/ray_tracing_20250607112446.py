...
class RayTracingSolver:
    ...
    def plot(self, show_solution=False, num_rays_show=10, ray_colorscale='Viridis'):
        ...
        if show_solution:
            # Plot rays
            num_total_rays = len(self.solution.rays)
            num_rays_show = min(num_rays_show, num_total_rays)
            indices = np.linspace(0, num_total_rays - 1, num_rays_show, dtype=int)
            for i in indices:
                ...
                log_powers = np.log10(powers + 1e-12)  # avoid log(0)
                ...
                for j in range(len(path) - 1):
                    fig.add_trace(go.Scatter3d(
                        ...
                        mode='lines',
                        line=dict(
                            color=[log_powers[j], log_powers[j]],  # constant color for the segment
                            colorscale=ray_colorscale,
                            cmin=np.min(log_powers),log_powers
                            cmax=np.max(powers),
                            width=3,
                            colorbar=dict(title='Ray Power') if (i == indices[0] and j == 0) else None  # Show colorbar once
                        ),
                        opacity=0.7,
                        showlegend=False,
                        name='Ray Path'
