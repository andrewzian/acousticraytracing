    ...
    def plot(self, show_solution=False, num_rays_show=10, ray_colorscale='Viridis'):
        ...
        for surf in room.surfaces:
            if surf.__class__.__name__ == "PlaneWall":
                ...
                # Use room bounds for x and y
                x_min, x_max = room.bounds['x']
                y_min, y_max = room.bounds['y']
                # Create a grid in x and y within room bounds
                x = np.linspace(x_min, x_max, 40)
                y = np.linspace(y_min, y_max, 40)
                X, Y = np.meshgrid(x, y)
                # Compute Z using the paraboloid equation
                Z = offset[2] + a * (X - offset[0])**2 + b * (Y - offset[1])**2 + c
                # Mask out Z values outside the room's z bounds
                z_min, z_max = room.bounds['z']
                Z_masked = np.where((Z >= z_min) & (Z <= z_max), Z, np.nan)
                fig.add_surface(
                    x=X, y=Y, z=Z_masked,
