...
class RayTracingSolution:
    ...
    def get_RT60(self, kernel_func=None, method="schroeder", dt=1e-3):
        ...
        Valid methods: noise, schroeder
        """
        if method not in ["noise", "schroeder"]:
            raise ValueError(f"Invalid method: {method}. Choose 'noise' or 'schroeder'.")
        RT60s = []
        
        if kernel_func:
            responses = self.get_responses(kernel_func, dt)
            if method == "noise":
                raise ValueError("Using noise method with kernel")
