    ...
    def plot(self, show_solution):
        ...
        for surf in room.surfaces:
            if surf.__class__.__name__ == "PlaneWall":
                ...
                v = np.array([1, 0, 0]) if not np.allclose(normal, [1,0,0]) else np.array([0,1,0])
                v1 = np.cross(normal, v)
                v1 /= np.linalg.norm(v1)
                v2 = np.cross(normal, v1)
                v2 /= np.linalg.norm(v2)
                # Make a square patch
                s = max([room.bounds[dim][1] - room.bounds[dim][0] for dim in room.bounds]) * 0.8
