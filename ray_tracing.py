"""
This code defines classes and methods for the ray tracing simulation, 
including RayTracingSolution and RayTracingSolver.
"""
from room import *
...
class RayTracingSolution:
    ...
    def get_responses(self, kernel_func, dt=1e-3, aggregate=False):
        ...
        for hits in hits_list:
            ...
            kernel /= np.sum(kernel) * dt  # Normalize kernel
...
class RayTracingSolver:
    ...
    def plot(self, show_solution=False, num_rays_show=10, ray_colorscale='Viridis', opacity=False, saveto=None):
        ...
        for surf in room.surfaces:
            if surf.__class__.__name__ == "PlaneWall":
                ...
                a, b, c = surf.paraboloid_coeffs
                offset = np.array(surf.offset)
                r_max = getattr(surf, "r_max", 5)
                u = np.linspace(0, 2 * np.pi, 30)
                r = np.linspace(0, r_max, 15)
                U, R = np.meshgrid(u, r)
                X = offset[0] + R * np.cos(U)
                Y = offset[1] + R * np.sin(U)
                Z = offset[2] + a * (X - offset[0])**2 + b * (Y - offset[1])**2 + c
        ...
        if receivers:
            for rec in receivers:
                ...
                fig.add_surface(
                    x=x, y=y, z=z,
                    opacity=0.7, colorscale='greys', showscale=False,
                    name='Receiver'
                )
        ...
        if show_solution:
            ...
            # Compute global min and max log power for color scaling
            all_log_powers = []
            for i in indices:
                ray = self.solution.rays[i]
                powers = np.array(ray.power_history)
                log_powers = np.log10(powers)
                if len(log_powers) != len(ray.path):
                    min_len = min(len(log_powers), len(ray.path))
                    log_powers = log_powers[:min_len]
                all_log_powers.extend(log_powers)
            global_min = np.min(all_log_powers)
            global_max = np.max(all_log_powers)
            if opacity:
                # Plot each ray as a single trace with opacity based on mean log-power
                for i in indices:
                    ray = self.solution.rays[i]
                    if len(ray.path) < 2:
                        continue
                    ...
                    # Set opacity based on mean log-power (normalize to [0.0, 1])
                    min_op, max_op = 0.0, 1.0
                    log_min, log_max = global_min, global_max
                    mean_power = np.mean(powers)
                    mean_log_power = np.mean(log_powers)
                    opacity_power = min_op + (mean_power - 10**log_min) / (10**log_max - 10**log_min + 1e-12) * (max_op - min_op)
                    opacity_log = min_op + (mean_log_power - log_min) / (log_max - log_min + 1e-12) * (max_op - min_op)
                    fig.add_trace(go.Scatter3d(
                ...
                for i in indices:
                    ...
                    fig.add_trace(go.Scatter3d(
                        ...
                        line=dict(
                            ...
                            cmin=global_min, 
                            cmax=global_max, 
                            colorbar=dict(title='Ray Log10(Power)', len=0.5) if ray is self.solution.rays[0] else None  # Show colorbar once
                        ),
                        opacity=0.7,
                        name=f'Ray {i}'
                    ))
        ...
        fig.update_layout(
            ...
            title="Acoustic Ray Tracing Visualization"
        )
        fig.show()

        # save to html
        if saveto:
            fig.write_html(f"{saveto}.html")
